...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function GitStatusIndicator(props) {
      ...
      var state = useState({ branch: null, dirty: false, branches: [], loading: true, error: null });
      var status = state[0];
      ...
      var load = useCallback(function () {
        ...
        fetch("/git-status" + q, { cache: "no-store" })
          ...
          .then(function (data) {
            setStatus({ branch: data.branch, dirty: data.dirty, branches: data.branches || [], loading: false, error: null });
          })
          .catch(function (e) {
            setStatus(function (prev) { return { branch: prev.branch, dirty: prev.dirty, branches: prev.branches, loading: false, error: String(e && e.message || e) }; });
          });
      }, [cwd]);
